@extends('AwfulMonitoring::layouts.master')
                    ...
                    <tr tabindex="0" class="focus:outline-none h-16 border border-gray-100 rounded">
                        ...
                        <td class="pl-24">
                            <div class="flex items-center">
                                ...
                                <p class="text-sm leading-none text-gray-600 ml-2">
                                    {{ $activityLog->causer?->name ?? 'Guest User' }}
                                </p>
                            </div>
                        </td>
                        <td class="pl-5">
                            <div class="flex items-center">
                                ...
                                <span class="inline-flex items-center rounded-md bg-{{ $activityLog->getTypeColor() }}-50 px-2 py-1 text-xs font-medium text-{{ $activityLog->getTypeColor() }}-700 ring-1 ring-inset ring-{{ $activityLog->getTypeColor() }}-600/10 ml-1">
                                    {{ $activityLog->event_name }}
                                </span>
                            </div>
                        </td>
                        <td class="pl-5">
                        ...
                        <td class="pl-5">
                            <button class="py-3 px-3 text-sm focus:outline-none leading-none text-blue-700 bg-blue-100 rounded">
                                {{ $activityLog->created_at->format('Y-m-d H:i A') }}
                            </button>
                        </td>
                        <td class="pl-4">
                            <form method="post" action="{{ route('awful_monitoring.activity_log_monitoring.delete', $activityLog->id) }}">
                                @csrf
                                @method('DELETE')
